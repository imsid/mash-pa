#!/bin/sh
# PA image entrypoint. Two modes, selected by MASH_DATABASE_URL:
#
...
if [ -z "${MASH_DATABASE_URL:-}" ]; then
    ...
    PGDATA="${PA_DATA_DIR:-/var/lib/pa}/pg"
    ...
    if [ ! -s "$PGDATA/PG_VERSION" ]; then
        echo "pa: initializing embedded Postgres at $PGDATA"
        as_postgres "$PG_BIN/initdb --no-locale -E UTF8 -D '$PGDATA'" > /dev/null
    fi

    echo "pa: starting embedded Postgres"
    ...
    as_postgres "$PG_BIN/psql -h $SOCKET_DIR -c 'CREATE DATABASE mash_pa OWNER mash'" 2> /dev/null || true

    export MASH_DATABASE_URL="postgresql://mash:mash@127.0.0.1:5432/mash_pa"
fi
