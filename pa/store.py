...
live in `~/.pa/hosts.json`: `pa compose` writes them, `pa hosts` and
`pa browse` list them, and `pa repl --host <id>` publishes them to the
deployment (idempotent PUTs) before connecting. On first use the file is
seeded with the `assistant` composition so the store works out of the box.
"""
...
def hosts_file_path() -> Path:
    home = Path(os.environ.get("PA_HOME", "~/.pa")).expanduser()
